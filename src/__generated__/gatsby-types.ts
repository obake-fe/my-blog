...
type templatesPostQueryVariables = Exact<{
  pathSlug: Scalars['String'];
}>;


type templatesPostQuery = { readonly markdownRemark: Maybe<(
    Pick<MarkdownRemark, 'html'>
    & { readonly frontmatter: Maybe<(
      Pick<MarkdownRemarkFrontmatter, 'date' | 'title' | 'tags'>
      & { readonly cover: Maybe<{ readonly childImageSharp: Maybe<{ readonly fluid: Maybe<Pick<ImageSharpFluid, 'base64' | 'aspectRatio' | 'src' | 'srcSet' | 'srcWebp' | 'srcSetWebp' | 'sizes'>>, readonly resize: Maybe<Pick<ImageSharpResize, 'src'>> }> }> }
    )> }
  )> };
...
type BlogQuery = { readonly allMarkdownRemark: { readonly edges: ReadonlyArray<{ readonly node: (
        ...
        & { readonly frontmatter: Maybe<(
          Pick<MarkdownRemarkFrontmatter, 'title' | 'path' | 'tags' | 'date'>
          & { readonly cover: Maybe<{ readonly childImageSharp: Maybe<{ readonly fluid: Maybe<Pick<ImageSharpFluid, 'tracedSVG' | 'aspectRatio' | 'src' | 'srcSet' | 'srcWebp' | 'srcSetWebp' | 'sizes'>> }> }> }
        )> }
      ) }> } };

type pagesIndexQueryVariables = Exact<{ [key: string]: never; }>;


type pagesIndexQuery = { readonly allMarkdownRemark: { readonly edges: ReadonlyArray<{ readonly node: (
        Pick<MarkdownRemark, 'id' | 'excerpt'>
        & { readonly frontmatter: Maybe<(
          Pick<MarkdownRemarkFrontmatter, 'title' | 'path' | 'tags' | 'date'>
          & { readonly cover: Maybe<{ readonly childImageSharp: Maybe<{ readonly fluid: Maybe<Pick<ImageSharpFluid, 'tracedSVG' | 'aspectRatio' | 'src' | 'srcSet' | 'srcWebp' | 'srcSetWebp' | 'sizes'>> }> }> }
